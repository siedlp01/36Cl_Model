function [bestAge, minRMSw ] = fit_first(debug, max_age, age_step, data,coll,slip,preexp,EL,epsilon,alpha,beta,gamma,Hfinal,rho_coll,rho_rock)
% Iterate from max_age to 0 (now) to determine the minimum root mean square
% and corresponding best age of the first event
...
for current = maxAge:-ageStep:0
    ...
    if minRMSw == 0 || seg1rmsw <= minRMSw 
        ...
        age(1) = bestAge;
        if debug == true
            disp(['Age ', num2str(age(1)), ' is the best fit for first event with ', num2str(minRMSw)]);
        end
        break;
    end
